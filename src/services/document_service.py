...
from ..models.code_document import CodeDocument, CodeDocumentCreate, CodeDocumentResponse
...
class DocumentProcessingService:
    ...
    async def get_repository_documents(
        ...
        try:
            ...
            language_stats = await self._get_language_statistics(repository_id, query_filter)
    ...
    async def get_document_content(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            mongodb = await get_mongodb_adapter()
            doc_data = await mongodb.find_document("code_documents", {"id": document_id})
    ...
    async def update_document_content(
        ...
        try:
            ...
            # Process new content
            processed_content = self._clean_content_for_embedding(new_content, doc_data["language"])
            ...
            success = await mongodb.update_document("code_documents", {"id": document_id}, updates)
    ...
    async def delete_document(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Delete document
            success = await mongodb.delete_document("code_documents", {"id": document_id})
    ...
    async def get_embedding_statistics(self, repository_id: Optional[UUID] = None) -> Dict[str, Any]:
    ...
    async def analyze_document_quality(self, repository_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_metrics["average_file_size"] = total_size // len(documents_data) if documents_data else 0
            quality_metrics["language_distribution"] = language_counts
            quality_metrics["embedding_coverage"] = (
                quality_metrics["documents_with_embeddings"] / quality_metrics["total_documents"] * 100
                if quality_metrics["total_documents"] > 0 else 0
            )
            
            # Calculate average quality score
            avg_quality = (
                sum(quality_metrics["content_quality_scores"]) / len(quality_metrics["content_quality_scores"])
                if quality_metrics["content_quality_scores"] else 0
            )
    ...
    async def _get_language_statistics(self, repository_id: UUID, base_query: Dict[str, Any]) -> Dict[str, int]:
    ...
    def _calculate_content_quality_score(self, content: str, processed_content: str, language: str) -> float:
    ...
    async def _process_new_file(self, repository_id: UUID, file_info: Dict[str, Any]) -> None:
        """Process a new file
    ...
    async def _process_modified_file(self, repository_id: UUID, file_info: Dict[str, Any]) -> None:
