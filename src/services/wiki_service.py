...
class WikiGenerationService:
    ...
    async def delete_wiki_for_repository(self, repository_id: UUID) -> bool:
        """Delete wiki structure for a repository.

        Args:
            repository_id: UUID of the repository

        Returns:
            True if wiki was deleted, False if no wiki existed
        """
        return await self._wiki_structure_repo.delete_one(
            {"repository_id": repository_id}
        )

    async def wiki_exists_for_repository(self, repository_id: UUID) -> bool:
        """Check if a wiki structure exists for a repository.

        Args:
            repository_id: UUID of the repository

        Returns:
            True if wiki exists, False otherwise
        """
