...
class WikiGenerationService:
    ...
    async def get_wiki_structure(
        ...
        try:
            ...
            # Get wiki structure
            wiki_data = await mongodb.find_document("wiki_structures", {"repository_id": str(repository_id)})
            ...
            if section_filter:
                wiki_structure = self._filter_wiki_by_section(wiki_structure, section_filter)
    ...
    def _filter_wiki_by_section(self, wiki_structure: WikiStructure, section_id: str) -> WikiStructure:
        ...
        try:
            ...
            filtered_pages = [
                page for page in wiki_structure.pages
                if page.id in target_section.pages
            ]
    ...
    async def _generate_documentation_files(self, wiki_data: Dict[str, Any]) -> List[Dict[str, str]]:
            ...
            for section_data in wiki_data.get("sections", []):
                ...
                for page_id in section_data.get("pages", []):
                    ...
                    if page_data:
                        ...
                        readme_content += f"- [{page_title}](docs/{page_id}.md) - {page_desc}\n"
...
"""
            ...
            for section_id in wiki_data.get("root_sections", []):
                ...
                if section_data:
                    nav_content += self._generate_section_nav(section_data, wiki_data, level=1)
    ...
    def _generate_section_nav(self, section_data: Dict[str, Any], wiki_data: Dict[str, Any], level: int = 1) -> str:
        ...
        for subsection_id in section_data.get("subsections", []):
            ...
            if subsection_data:
                nav_content += self._generate_section_nav(subsection_data, wiki_data, level + 1)
    ...
    async def _create_git_pull_request(
        ...
        try:
            ...
            pr_title = title or f"📚 Update documentation for {repository.org}/{repository.name}"
            pr_description = description or f"""## AutoDoc Documentation Update
...
"""
    ...
    async def get_wiki_generation_status(self, repository_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            status_result = await wiki_agent.get_wiki_generation_status(str(repository_id))
    ...
    async def regenerate_wiki_page(
        ...
        try:
            ...
            new_content = await wiki_agent._generate_page_content(page_data, str(repository_id))
