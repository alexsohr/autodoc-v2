...
from ..models.chat import (
    ChatSession, Question, Answer, Citation,
    QuestionRequest, QuestionResponse, AnswerResponse, QuestionAnswer,
...
class ChatService:
    ...
    async def get_chat_session(self, repository_id: UUID, session_id: UUID) -> Dict[str, Any]:
    ...
    async def delete_chat_session(self, repository_id: UUID, session_id: UUID) -> Dict[str, Any]:
        ...
        try:
            ...
            async with await mongodb.client.start_session() as db_session:
                async with db_session.start_transaction():
                    ...
                    questions = await mongodb.find_documents("questions", {"session_id": str(session_id)})
                    for question in questions:
                        await mongodb.delete_document("answers", {"question_id": question["id"]}, session=db_session)
                    
                    await mongodb.delete_document("questions", {"session_id": str(session_id)}, session=db_session)
                    
                    # Delete session
                    await mongodb.delete_document("chat_sessions", {"id": str(session_id)}, session=db_session)
    ...
    async def ask_question(
        ...
        try:
            ...
            question = Question(
                ...
                content=question_request.content
            )
            ...
            question.context_nodes = [ctx.get("file_path", "") for ctx in context_documents]
    ...
    async def get_conversation_history(
        ...
        try:
            ...
            for question_data in questions_data:
                ...
                answer_data = await mongodb.find_document("answers", {"question_id": str(question.id)})
                ...
                if answer_data:
                    ...
                    question_answer_pairs.append(QuestionAnswer(
            ...
            return {
                "status": "success",
                "session_id": str(session_id),
                "questions_and_answers": [qa.model_dump() for qa in question_answer_pairs],
    ...
    async def get_chat_statistics(self, repository_id: Optional[UUID] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_questions_per_session": total_questions / total_sessions if total_sessions > 0 else 0,
                "recent_activity": [
                    {
