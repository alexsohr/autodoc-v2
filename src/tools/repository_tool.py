...
from ..models.repository import Repository, RepositoryProvider, AnalysisStatus
...
class RepositoryCloneInput(BaseModel):
    ...
    target_directory: Optional[str] = Field(default=None, description="Target directory for clone")
...
class RepositoryTool(BaseTool):
    ...
    async def clone_repository(
        ...
        try:
            ...
            try:
                ...
                process.kill()
                await process.wait()
                raise TimeoutError(f"Repository clone timeout after {self._clone_timeout} seconds")
    ...
    async def _get_repository_info(self, repo_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            origin_url = stdout.decode('utf-8').strip() if process.returncode == 0 else ""
            ...
            stdout, _ = await process.communicate()
            current_branch = stdout.decode('utf-8').strip() if process.returncode == 0 else ""
            ...
            stdout, _ = await process.communicate()
            commit_sha = stdout.decode('utf-8').strip() if process.returncode == 0 else ""
            ...
            stdout, _ = await process.communicate()
            commit_count = int(stdout.decode('utf-8').strip()) if process.returncode == 0 else 0
        ...
        try:
            ...
            for item in repo_path.rglob("*"):
                if item.is_dir():
                    ...
                    elif file_name.startswith("readme") or file_name.endswith((".md", ".rst", ".txt")):
                        if "doc" in relative_path.lower() or file_name.startswith("readme"):
    ...
    def _calculate_language_stats(self, files: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "primary_language": max(language_sizes.keys(), key=language_sizes.get) if language_sizes else "unknown"
        }
