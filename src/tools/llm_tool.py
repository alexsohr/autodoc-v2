...
from ..models.config import LLMProvider, LLMConfig
...
class LLMTool(BaseTool):
    ...
    name: str = "llm_tool"
    description: str = "Tool for multi-provider LLM text generation, chat, and streaming"
    ...
    async def generate_structured(
        ...
        try:
            ...
            return {
                ...
                "schema_type": schema.__name__ if hasattr(schema, '__name__') else str(schema),
                "provider": provider or "default",
                "generation_time": generation_time,
                "timestamp": end_time.isoformat()
            }
...
```"""
            ...
            structured_analysis = self._parse_analysis_response(analysis_text, analysis_type)
...
7. License information"""
            ...
            base_prompt = doc_prompts.get(documentation_type, doc_prompts["api_reference"])
    ...
    def _get_llm_provider(self, provider: Optional[str] = None) -> Optional[BaseLanguageModel]:
        ...
        try:
            ...
            for line in lines:
                ...
                if re.match(r'^\d+\.', stripped) or stripped.startswith('#'):
                    ...
                    section_title = re.sub(r'[^\w\s]', '', section_title).replace(' ', '_')
    ...
    def _parse_documentation_sections(self, documentation_text: str) -> List[Dict[str, str]]:
        """Parse documentation into sections
...
5. Be precise and technical when appropriate"""
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            healthy_count = sum(1 for p in provider_health.values() if p["status"] == "healthy")
