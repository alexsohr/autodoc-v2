...
class EmbeddingGenerateInput(BaseModel):
    ...
    provider: Optional[str] = Field(
        default=None, description="Embedding provider (defaults to configured)"
    )
...
class EmbeddingTool(BaseTool):
    ...
    def _setup_embedding_providers(self, settings) -> None:
        ...
        try:
            ...
            if settings.google_api_key:
                self._embedding_providers[LLMProvider.GEMINI] = (
            ...
            logger.info(
                f"Initialized {len(self._embedding_providers)} embedding providers"
            )
    ...
    async def generate_embeddings(
        ...
        try:
            if not texts:
                return {"status": "success", "embeddings": [], "count": 0}
    ...
    async def generate_and_store_embeddings(
        ...
        try:
            if not documents:
                return {"status": "success", "processed_count": 0, "failed_count": 0}
            ...
            embedding_result = await self.generate_embeddings(
                texts=texts, provider=provider, batch_size=batch_size
            )
            ...
            for i, (doc_id, embedding) in enumerate(zip(document_ids, embeddings)):
                if embedding is not None:
                    ...
                    if store_result["status"] == "success":
                        ...
                        failed_count += 1
                        logger.error(
                            f"Failed to store embedding for document {doc_id}: {store_result.get('error')}"
                        )
                else:
                    failed_count += 1
    ...
    async def batch_process_documents(
        ...
        try:
            ...
            result = await self.generate_and_store_embeddings(
                documents=docs_to_process, provider=provider, batch_size=batch_size
            )
    ...
    async def reprocess_embeddings(
        self, repository_id: str, provider: Optional[str] = None, force: bool = False
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.batch_process_documents(
                documents=code_documents, provider=provider
            )
