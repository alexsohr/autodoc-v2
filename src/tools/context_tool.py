...
class ContextSearchInput(BaseModel):
    ...
    file_path_filter: Optional[str] = Field(
        default=None, description="File path pattern filter"
    )
    ...
    search_type: Optional[str] = Field(
        default="hybrid", description="Search type: vector, text, or hybrid"
    )
...
class ContextRankInput(BaseModel):
    ...
    max_contexts: Optional[int] = Field(
        default=5, description="Maximum contexts to return"
    )
    ranking_strategy: Optional[str] = Field(
        default="relevance", description="Ranking strategy"
    )
...
class ContextTool(BaseTool):
    ...
    async def _extract_context_info(
        ...
        context_info = {
            ...
            "content_preview": self._generate_content_preview(
        ...
        if document.language in ["python", "javascript", "typescript", "java", "go"]:
            structure_info = self._analyze_code_structure(
                document.content, document.language
            )
    ...
    def _analyze_js_structure(self, lines: List[str]) -> Dict[str, List[str]]:
        ...
        for line in lines:
            ...
            # Imports
                ...
                stripped.startswith(("import ", "const ", "let ", "var "))
                and "require(" in stripped
            ):
                imports.append(stripped)

        return {"functions": functions, "classes": classes, "imports": imports[:10]}
    ...
    async def rank_contexts(
        ...
        try:
            if not contexts:
                return {"status": "success", "ranked_contexts": [], "count": 0}
    ...
    async def _rank_by_diversity(
        self, contexts: List[Dict[str, Any]], max_contexts: int
    ) -> List[Dict[str, Any]]:
        ...
        sorted_contexts = sorted(
            contexts, key=lambda x: x.get("similarity_score", 0), reverse=True
        )
    ...
    async def extract_code_snippets(
        self, contexts: List[Dict[str, Any]], query: str, max_snippet_length: int = 500
    ) -> Dict[str, Any]:
    ...
    async def _extract_relevant_snippet(
        self, content: str, query: str, file_path: str, language: str, max_length: int
        ...
        try:
            ...
            return {
                ...
                "matching_words": list(
                    query_words & set(re.findall(r"\w+", snippet_text.lower()))
                ),
            }
    ...
    async def hybrid_search(
        ...
        try:
            ...
            snippet_result = await self.extract_code_snippets(
                contexts=rank_result["ranked_contexts"], query=query
            )
