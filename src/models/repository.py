...
class Repository(BaseModel):
    ...
    analysis_status: AnalysisStatus = Field(default=AnalysisStatus.PENDING, description="Current analysis status")
    commit_sha: Optional[str] = Field(default=None, description="Last analyzed commit SHA")
    
    # Webhook configuration fields
    webhook_configured: bool = Field(default=False, description="Whether webhook is configured")
    webhook_secret: Optional[str] = Field(default=None, description="Secret for validating webhook signatures")
    subscribed_events: List[str] = Field(default_factory=list, description="Events that trigger documentation updates")
    last_webhook_event: Optional[datetime] = Field(default=None, description="Timestamp of last received webhook event")
    ...
    def validate_subscribed_events(cls, v: List[str]) -> List[str]:
        ...
        valid_events = {
            "push", "pull_request", "merge", "pullrequest:fulfilled", 
            "repo:push", "repository:push", "merge_request"
        }
    ...
    def validate_webhook_configuration(self) -> "Repository":
        ...
        if self.webhook_configured and not self.webhook_secret:
            raise ValueError("Webhook secret must be provided when webhook is configured")
    ...
    def update_analysis_status(self, status: AnalysisStatus, commit_sha: Optional[str] = None) -> None:
...
class RepositoryCreate(BaseModel):
    ...
    url: str = Field(description="Repository URL")
    branch: Optional[str] = Field(default=None, description="Target branch (defaults to repository default)")
    provider: Optional[RepositoryProvider] = Field(default=None, description="Repository provider (auto-detected if not specified)")
...
class RepositoryUpdate(BaseModel):
    ...
    webhook_configured: Optional[bool] = Field(default=None, description="Webhook configuration status")
    webhook_secret: Optional[str] = Field(default=None, description="Webhook secret")
    subscribed_events: Optional[List[str]] = Field(default=None, description="Subscribed webhook events")
