...
class Citation(BaseModel):
    ...
    def __repr__(self) -> str:
        return f"Citation(file_path={self.file_path}, commit_sha={self.commit_sha[:8]}...)"
...
class Answer(BaseModel):
    ...
    citations: List[Citation] = Field(default_factory=list, description="Source code references")
    ...
    # Audit field
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Answer timestamp")
...
class Question(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=lambda: datetime.now(timezone.utc), description="Question timestamp")
    
    # Context information
    context_nodes: List[str] = Field(default_factory=list, description="Relevant analysis node IDs used for context")
...
class ChatSession(BaseModel):
    ...
    # Statistics
    message_count: int = Field(default=0, description="Number of questions in this session")
...
class QuestionRequest(BaseModel):
    ...
    context_hint: Optional[str] = Field(default=None, description="Optional hint about what part of codebase to focus on")
...
class QuestionResponse(BaseModel):
    ...
    context_files: List[str] = Field(description="File paths used for context via semantic search")
...
class ConversationHistory(BaseModel):
    ...
    session_id: UUID = Field(description="Session ID")
    questions_and_answers: List[QuestionAnswer] = Field(description="Q&A pairs in chronological order")
