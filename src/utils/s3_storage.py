...
from .storage_adapters import StorageAdapter, StorageError, FileNotFoundError, PermissionError, StorageFullError, FileMetadata
...
class S3StorageAdapter(StorageAdapter):
    ...
    def __init__(self, config: StorageConfig):
        ...
        self.bucket_name = config.get_connection_param("bucket_name") or config.base_path
        ...
        self.endpoint_url = config.get_connection_param("endpoint_url")  # For S3-compatible services
        
        # S3 client configuration
        self.storage_class = config.get_connection_param("storage_class", "STANDARD")
        self.server_side_encryption = config.get_connection_param("server_side_encryption", "AES256")
        self.versioning_enabled = config.get_connection_param("versioning_enabled", True)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            # Test bucket access
            await self._run_in_executor(self.s3_client.head_bucket, Bucket=self.bucket_name)
    ...
    async def read_text_file(self, path: str, encoding: str = "utf-8") -> str:
        ...
        try:
            ...
            raise StorageError(f"Error decoding file {path} with encoding {encoding}: {e}")
    
    async def write_file(self, path: str, content: Union[bytes, str], metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    async def list_files(self, path: str = "", recursive: bool = False, pattern: Optional[str] = None) -> List[str]:
            ...
            async for page in self._paginate_async(pages):
                if 'Contents' in page:
                    for obj in page['Contents']:
                        ...
                        if pattern is None or fnmatch.fnmatch(relative_path.split('/')[-1], pattern):
    ...
    async def list_directories(self, path: str = "", recursive: bool = False) -> List[str]:
    ...
    async def read_file_stream(self, path: str, chunk_size: int = 8192) -> AsyncIterator[bytes]:
    ...
    async def write_file_stream(self, path: str, content_stream: AsyncIterator[bytes], metadata: Optional[Dict[str, Any]] = None) -> None:
    ...
    async def copy_file(self, source_path: str, dest_path: str) -> None:
        ...
        try:
            ...
            if error_code == 'NoSuchKey':
                ...
                raise PermissionError(f"Permission denied copying {source_path} to {dest_path}")
            else:
                raise StorageError(f"Error copying file {source_path} to {dest_path}: {e}")
