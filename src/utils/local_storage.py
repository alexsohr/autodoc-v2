...
from .storage_adapters import StorageAdapter, StorageError, FileNotFoundError, PermissionError, FileMetadata
    ...
    async def move_file(self, source_path: str, dest_path: str) -> None:
        ...
        try:
            ...
            if e.errno == 2:  # No such file or directory
                ...
                raise PermissionError(f"Permission denied moving {source_path} to {dest_path}")
            else:
                raise StorageError(f"Error moving file {source_path} to {dest_path}: {e}")
