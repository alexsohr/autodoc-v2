...
class Settings(BaseSettings):
    ...
    mongodb_url: str = Field(
        default="mongodb://localhost:27017", description="MongoDB URL"
    )
    mongodb_database: str = Field(default="autodoc_dev", description="MongoDB database")
    mongodb_max_connections: int = Field(
        default=10, description="MongoDB max connections"
    )
    mongodb_min_connections: int = Field(
        default=1, description="MongoDB min connections"
    )

    # Storage settings
    storage_type: StorageType = Field(
        default=StorageType.LOCAL, description="Storage type"
    )
    ...
    webhook_secret_key: str = Field(
        default="your-webhook-secret-key", description="Webhook secret key"
    )
    github_webhook_secret: Optional[str] = Field(
        default=None, description="GitHub webhook secret"
    )
    bitbucket_webhook_secret: Optional[str] = Field(
        default=None, description="Bitbucket webhook secret"
    )

    # Performance settings
    max_concurrent_analyses: int = Field(
        default=3, description="Maximum concurrent analyses"
    )
    ...
    clone_timeout_seconds: int = Field(
        default=300, description="Repository clone timeout in seconds"
    )
