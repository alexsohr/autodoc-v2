...
class DocumentProcessingAgent:
    ...
    async def process_repository(
        ...
        try:
            ...
            initial_state: DocumentProcessingState = {
                ...
                "messages": [HumanMessage(content=f"Process repository: {repository_url}")]
            }
            ...
            logger.error(f"Document processing failed for repository {repository_id}: {e}")
    ...
    async def _clone_repository_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            if clone_result["status"] != "success":
                state["error_message"] = f"Repository clone failed: {clone_result.get('error', 'Unknown error')}"
                return state
    ...
    async def _discover_files_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        """Discover files node"""
        try:
            ...
            discovery_result = await repository_tool._arun(
                "discover_files",
                repository_path=state["clone_path"]
            )
            
            if discovery_result["status"] != "success":
                state["error_message"] = f"File discovery failed: {discovery_result.get('error', 'Unknown error')}"
                return state
    ...
    async def _process_content_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            for i, file_info in enumerate(state["discovered_files"]):
                try:
                    ...
                    processed_content = self._clean_content_for_embedding(content, file_info["language"])
    ...
    async def _generate_embeddings_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            if not state["processed_documents"]:
                state["error_message"] = "No processed documents for embedding generation"
                return state
            ...
            if embedding_result["status"] != "success":
                state["error_message"] = f"Embedding generation failed: {embedding_result.get('error', 'Unknown error')}"
                return state
    ...
    async def _store_documents_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            ...
            for doc_data in state["processed_documents"]:
                try:
                    ...
                    # Store in database
                    await mongodb.insert_document("code_documents", code_document.model_dump())
                    stored_count += 1
                    
                except Exception as e:
                    logger.warning(f"Failed to store document {doc_data.get('id')}: {e}")
                    continue
            ...
            # Add success message
            state["messages"].append(AIMessage(
                content=f"Stored {stored_count} documents in database"
            ))
    ...
    async def _cleanup_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        """Cleanup node"""
        try:
            ...
            if state["clone_path"]:
                ...
                if cleanup_result["status"] == "success":
                    state["messages"].append(AIMessage(
                        content="Repository cleanup completed"
                    ))
    ...
    async def _handle_error_node(self, state: DocumentProcessingState) -> DocumentProcessingState:
        ...
        try:
            # Log error details
            logger.error(f"Document processing failed for repository {state['repository_id']}: {state.get('error_message')}")
    ...
    def _clean_content_for_embedding(self, content: str, language: str) -> str:
        ...
        try:
            ...
            if language == "python":
                ...
                cleaned = re.sub(r'""".*?"""', lambda m: m.group(0).replace('"""', ''), cleaned, flags=re.DOTALL)
                cleaned = re.sub(r"'''.*?'''", lambda m: m.group(0).replace("'''", ''), cleaned, flags=re.DOTALL)
    ...
    async def get_processing_status(self, repository_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "embedding_coverage": (embedding_count / doc_count * 100) if doc_count > 0 else 0,
