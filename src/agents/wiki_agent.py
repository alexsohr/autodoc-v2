...
from ..models.wiki import WikiStructure, WikiPageDetail, WikiSection, PageImportance
...
class WikiPageSchema(BaseModel):
    ...
    file_paths: List[str] = Field(description="List of relevant file paths from the repository")
    related_pages: List[str] = Field(default_factory=list, description="List of related page IDs")
...
class WikiSectionSchema(BaseModel):
    ...
    pages: List[str] = Field(default_factory=list, description="List of page IDs in this section")
...
- Structure the document logically for easy understanding by other developers."""
    ...
    async def generate_wiki(
        ...
        try:
            ...
            if not force_regenerate:
                ...
                existing_wiki = await mongodb.find_document("wiki_structures", {"repository_id": repository_id})
                if existing_wiki:
                    return {
                        "status": "exists",
            ...
            initial_state: WikiGenerationState = {
                ...
                "messages": [HumanMessage(content=f"Generate wiki for repository: {repository_id}")]
            }
    ...
    async def _analyze_repository_node(self, state: WikiGenerationState) -> WikiGenerationState:
        ...
        try:
            ...
            state["messages"].append(AIMessage(
                content=f"Analyzed repository with {len(documents)} files"
            ))
    ...
    async def _generate_structure_node(self, state: WikiGenerationState) -> WikiGenerationState:
    ...
    async def _generate_pages_node(self, state: WikiGenerationState) -> WikiGenerationState:
        ...
        try:
            ...
            if not state["wiki_structure"]:
                state["error_message"] = "No wiki structure available for page generation"
                return state
    ...
    async def _store_wiki_node(self, state: WikiGenerationState) -> WikiGenerationState:
        ...
        try:
            ...
            await mongodb.delete_document("wiki_structures", {"repository_id": state["repository_id"]})
    ...
    async def _handle_error_node(self, state: WikiGenerationState) -> WikiGenerationState:
        """Handle error node"""
        try:
            # Log error details
            logger.error(f"Wiki generation failed for repository {state['repository_id']}: {state.get('error_message')}")
    ...
    def _format_tree_structure(self, structure: Dict[str, Any], indent: str = "") -> str:
    ...
    async def _generate_structured_wiki_structure(
        ...
        try:
            ...
            if generation_result["status"] != "success":
                logger.error(f"Structured generation failed: {generation_result.get('error')}")
                return None
            ...
            if not wiki_structure.get("root_sections"):
                ...
                root_sections = [
                    section["id"] for section in wiki_structure.get("sections", [])
    ...
    async def _generate_pages_node(self, state: WikiGenerationState) -> WikiGenerationState:
        ...
        try:
            ...
            for i, page_info in enumerate(pages):
                try:
                    page_content = await self._generate_page_content(page_info, state["repository_id"])
    ...
    async def _generate_page_content(self, page_info: Dict[str, Any], repository_id: str) -> Optional[str]:
        ...
        try:
            ...
            page_prompt = self.page_prompt_template.format(
                ...
                file_list_markdown=file_list_markdown
            )
            
            # Add source file contents to prompt
            source_contents = []
            for file_info in relevant_files[:5]:  # Limit to 5 files for token efficiency
            ...
            if generation_result["status"] == "success":
                ...
                logger.error(f"Page content generation failed: {generation_result.get('error')}")
                return None
    ...
    async def get_wiki_generation_status(self, repository_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            wiki = await mongodb.find_document("wiki_structures", {"repository_id": repository_id})
            
            if wiki:
                # Count pages with content
                pages_with_content = sum(
                    1 for page in wiki.get("pages", [])
