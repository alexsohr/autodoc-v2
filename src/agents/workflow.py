...
from ..agents.document_agent import document_agent, DocumentProcessingAgent
from ..agents.wiki_agent import wiki_agent, WikiGenerationAgent
...
class WorkflowOrchestrator:
    ...
    def _create_workflows(self) -> Dict[str, StateGraph]:
        ...
        workflows[WorkflowType.DOCUMENT_PROCESSING] = self._create_document_processing_workflow()
        
        # Wiki generation workflow
        workflows[WorkflowType.WIKI_GENERATION] = self._create_wiki_generation_workflow()
        
        # Incremental update workflow
        workflows[WorkflowType.INCREMENTAL_UPDATE] = self._create_incremental_update_workflow()
    ...
    def _create_incremental_update_workflow(self) -> StateGraph:
        ...
        workflow.add_node("process_changed_documents", self._process_changed_documents_node)
    ...
    async def execute_workflow(
        ...
        try:
            ...
            initial_state: WorkflowState = {
                ...
                "messages": [HumanMessage(content=f"Execute {workflow_type.value} workflow for repository {repository_id}")]
            }
            
            # Execute workflow with checkpointing
            config = {"configurable": {"thread_id": f"{workflow_type.value}_{repository_id}"}}
            ...
            return {
                ...
                "execution_time": self._calculate_execution_time(result.get("start_time", ""))
            }
    ...
    async def _validate_repository_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            repository = await mongodb.find_document("repositories", {"id": state["repository_id"]})
    ...
    async def _process_documents_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if processing_result["status"] != "completed":
                state["error_message"] = f"Document processing failed: {processing_result.get('error_message', 'Unknown error')}"
                return state
        ...
        try:
            ...
            if not state["force_update"]:
                ...
                if existing_wiki:
                    ...
                    state["messages"].append(AIMessage(
                        content="Using existing wiki structure"
                    ))
            ...
            if wiki_result["status"] not in ["completed", "exists"]:
                state["error_message"] = f"Wiki generation failed: {wiki_result.get('error_message', 'Unknown error')}"
                return state
    ...
    async def _process_changed_documents_node(self, state: WorkflowState) -> WorkflowState:
    ...
    async def _retrieve_context_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if context_result["status"] != "success":
                state["error_message"] = f"Context retrieval failed: {context_result.get('error', 'Unknown error')}"
                return state
    ...
    async def _generate_response_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            ...
            if answer_result["status"] != "success":
                state["error_message"] = f"Response generation failed: {answer_result.get('error', 'Unknown error')}"
                return state
    ...
    async def _handle_error_node(self, state: WorkflowState) -> WorkflowState:
        ...
        try:
            # Log error details
            logger.error(f"Workflow {state['workflow_type']} failed for repository {state['repository_id']}: {state.get('error_message')}")
            ...
            if state["workflow_type"] in [WorkflowType.FULL_ANALYSIS.value, WorkflowType.DOCUMENT_PROCESSING.value]:
                await self._update_repository_status(state["repository_id"], AnalysisStatus.FAILED, state.get("error_message"))
    ...
    async def get_workflow_status(self, workflow_type: WorkflowType, repository_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            mongodb = await get_mongodb_adapter()
            repository = await mongodb.find_document("repositories", {"id": repository_id})
            ...
            if workflow_type == WorkflowType.FULL_ANALYSIS:
                doc_count = await mongodb.count_documents("code_documents", {"repository_id": repository_id})
                wiki_exists = await mongodb.find_document("wiki_structures", {"repository_id": repository_id})
