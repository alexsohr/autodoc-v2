...
class TestRepositoryRegistrationWorkflow:
    ...
    @pytest.mark.asyncio
    async def test_complete_repository_registration_workflow(self, async_client: AsyncClient):
        ...
        with patch('src.services.repository_service.RepositoryService.analyze_repository') as mock_analyze:
    ...
    @pytest.mark.asyncio
    async def test_repository_registration_with_auto_detection(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_repository_registration_with_custom_branch(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_repository_registration_duplicate_handling(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_repository_registration_unsupported_provider(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_repository_analysis_timeout_handling(self, async_client: AsyncClient):
    ...
    @pytest.mark.asyncio
    async def test_repository_analysis_force_reanalysis(self, async_client: AsyncClient):
        ...
        force_response = await async_client.post(
            f"/repositories/{repository_id}/analyze",
            json=force_analysis_payload
        )
        
        assert force_response.status_code == status.HTTP_202_ACCEPTED

    @pytest.mark.asyncio
    async def test_repository_analysis_concurrent_requests(self, async_client: AsyncClient):
        ...
        success_count = sum(1 for r in responses if hasattr(r, 'status_code') and r.status_code == status.HTTP_202_ACCEPTED)
        conflict_count = sum(1 for r in responses if hasattr(r, 'status_code') and r.status_code == status.HTTP_409_CONFLICT)
    ...
    async def test_repository_deletion_workflow(self, async_client: AsyncClient):
        ...
        if response.status_code == status.HTTP_201_CREATED:
            ...
            # Delete repository
            delete_response = await async_client.delete(f"/repositories/{repository_id}")
