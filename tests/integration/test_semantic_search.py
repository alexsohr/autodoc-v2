...
class TestSemanticSearchAndRAG:
    ...
    async def test_complete_semantic_search_workflow(self, async_client: AsyncClient):
            ...
            with patch("src.tools.embedding_tool.EmbeddingTool") as mock_embedding_tool:
                ...
                mock_processed_docs = [
                    ...
                    },
                    {
                        ...
                        "metadata": {
                            "test_functions": ["test_login", "test_token_validation"]
                        },
                    },
                ]

                mock_doc_instance.get_processed_documents = AsyncMock(
                    return_value=mock_processed_docs
                )
        ...
        with patch("src.tools.context_tool.ContextTool") as mock_context_tool:
            ...
            for query_data in search_queries:
                    ...
                    for doc in mock_processed_docs:
                        ...
                        if score > 0:
                            relevant_docs.append(
                                {"document": doc, "score": min(score, 1.0)}
                            )
                ...
                # Perform search
                search_results = await mock_context_instance.similarity_search(
                    query_data["query"]
                )
        ...
        with patch("src.services.chat_service.ChatService") as mock_chat_service:
            ...
            for test_case in rag_test_cases:
                ...
                mock_chat_instance.retrieve_context = AsyncMock(
                    return_value=mock_context_docs
                )
    ...
    @pytest.mark.asyncio
    async def test_embedding_quality_and_similarity_search(
        self, async_client: AsyncClient
    ):
        ...
        with patch("src.tools.embedding_tool.EmbeddingTool") as mock_embedding_tool:
            ...
            def mock_similarity_search(query_embedding, k=5):
                ...
                for doc in test_documents:
                    ...
                    dot_product = sum(
                        a * b for a, b in zip(query_embedding[:5], doc_embedding[:5])
                    )
                    ...
                    similarities.append({"document": doc, "score": similarity})
            ...
            for test in similarity_tests:
                ...
                results = await mock_embedding_instance.similarity_search(
                    query_embedding, k=3
                )
                ...
                assert any(
                    cat in test["expected_top_categories"] for cat in top_categories
                )
    ...
    async def test_multi_language_semantic_search(self, async_client: AsyncClient):
        ...
        with patch("src.services.document_service.DocumentService") as mock_doc_service:
            ...
            with patch("src.tools.embedding_tool.EmbeddingTool") as mock_embedding_tool:
                ...
                mock_embedding_instance.generate_embedding = AsyncMock(
                    side_effect=lambda content, lang=None: generate_language_aware_embedding(
                        content, lang
                    )
                )
                ...
                with patch("src.tools.context_tool.ContextTool") as mock_context_tool:
                    ...
                    for query_test in cross_lang_queries:
                        ...
                        result_languages = set(
                            r["document"]["language"] for r in results
                        )
    ...
    @pytest.mark.asyncio
    async def test_semantic_search_performance_and_scalability(
        self, async_client: AsyncClient
    ):
        ...
        with patch("src.services.document_service.DocumentService") as mock_doc_service:
            ...
            with patch("src.services.data_access.MongoDBAdapter") as mock_mongo:
                ...
                mock_mongo_instance.vector_search = AsyncMock(
        ...
        with patch("src.services.chat_service.ChatService") as mock_chat_service:
            ...
            def mock_filter_and_rank_context(candidates, query, max_context=5):
                ...
                for candidate in candidates:
                    ...
                    if (
                        ...
                        or metadata.get("documentation_quality", 0) < 0.2
                    ):
                        continue
                    ...
                    if (
                        ...
                        and metadata["file_type"] == "documentation"
                    ):
                        composite_score *= 1.2
            ...
            for test in context_tests:
                ...
                filtered_context = await mock_chat_instance.filter_and_rank_context(
                    mock_context_candidates, test["question"], max_ctx=3
                )

                # Verify filtering quality
                assert len(filtered_context) <= 3
                assert all(
                    ctx["composite_score"] >= test["min_quality_score"]
                ...
                top_types = [
                    ctx["metadata"]["file_type"] for ctx in filtered_context[:2]
                ]
                ...
                mock_answer = {
                    ...
                    "confidence_score": min(
                        0.95,
                        sum(ctx["composite_score"] for ctx in filtered_context)
                        / len(filtered_context),
                    ),
                    "generation_time": 1.0,
                    "context_quality_score": sum(
                        ctx["composite_score"] for ctx in filtered_context
                    )
                    / len(filtered_context),
                }
