...
class TestChatWorkflow:
    ...
    async def test_complete_chat_workflow(self, async_client: AsyncClient):
        ...
        with patch('src.services.chat_service.ChatService') as mock_service:
            ...
            mock_instance.find_relevant_context = AsyncMock(return_value=mock_context_files)
    ...
    async def test_chat_conversation_history(self, async_client: AsyncClient):
        ...
        with patch('src.services.chat_service.ChatService') as mock_service:
            ...
            questions_and_answers = [
                ("What is the main purpose of this project?", "This project is a web API..."),
                ("How do I run the tests?", "To run tests, use pytest..."),
                ("What are the main dependencies?", "The main dependencies are FastAPI..."),
            ]
    ...
    async def test_chat_context_relevance_and_search(self, async_client: AsyncClient):
        ...
        with patch('src.services.chat_service.ChatService') as mock_service:
            ...
            for question_data in test_questions:
                ...
                mock_instance.find_relevant_context = AsyncMock(return_value=relevant_files)
                ...
                mock_answer = {
                    ...
                    "citations": [
                        {
                            "file_path": relevant_files[0] if relevant_files else "src/main.py",
                            "line_start": 1,
                            "line_end": 10,
                            "commit_sha": "abc123",
                ...
                if qa_response.status_code == status.HTTP_201_CREATED:
                    ...
                    if context_files:
                        ...
                        assert any(keyword in context_text for keyword in expected_keywords)
        ...
        for scenario in error_scenarios:
            if "exception" in scenario:
                ...
                with patch('src.services.chat_service.ChatService') as mock_service:
                    ...
                    mock_instance.generate_answer = AsyncMock(side_effect=scenario["exception"])
