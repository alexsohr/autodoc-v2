...
class TestWebhookProcessingWorkflow:
    ...
    async def test_complete_github_webhook_workflow(self, async_client: AsyncClient):
        ...
        webhook_config_response = await async_client.put(
            f"/repositories/{repository_id}/webhook",
            json=webhook_config_payload
        )
        ...
        signature = self._generate_github_signature(push_payload, webhook_config_payload["webhook_secret"])
        ...
        with patch('src.services.repository_service.RepositoryService') as mock_repo_service:
            with patch('src.services.document_service.DocumentService') as mock_doc_service:
                with patch('src.services.wiki_service.WikiService') as mock_wiki_service:
                    
                    # Mock repository lookup
                    mock_repo_instance = MagicMock()
                    mock_repo_service.return_value = mock_repo_instance
                    mock_repo_instance.find_by_url = AsyncMock(return_value={
                        "id": repository_id,
                    ...
                    mock_doc_instance.process_changed_files = AsyncMock(return_value={
                        "processed_files": 3,
                        "added_files": 2,
                        "modified_files": 2,
                        "removed_files": 1
                    ...
                    mock_wiki_instance.update_wiki_for_changes = AsyncMock(return_value={
                    ...
                    # Mock PR creation
                    mock_wiki_instance.create_documentation_pr = AsyncMock(return_value={
                        "pull_request_url": "https://github.com/test-org/webhook-repo/pull/456",
                        "branch_name": "autodoc/webhook-update-20231201-123456",
                        "files_changed": ["docs/overview.md", "docs/api-reference.md", "docs/new-feature.md"],
    ...
    @pytest.mark.asyncio
    async def test_github_pull_request_webhook_workflow(self, async_client: AsyncClient):
        ...
        signature = self._generate_github_signature(pr_payload, webhook_config_payload["webhook_secret"])
        ...
        # Mock PR processing
        with patch('src.services.webhook_service.WebhookService') as mock_webhook_service:
            mock_instance = MagicMock()
            mock_webhook_service.return_value = mock_instance
            
            mock_instance.process_github_webhook = AsyncMock(return_value={
                "status": "processed",
    ...
    async def test_bitbucket_webhook_workflow(self, async_client: AsyncClient):
        ...
        bitbucket_payload = {
            ...
            "repository": {
                ...
                "links": {
                    ...
                    "html": {
                        "href": "https://bitbucket.org/test-org/bitbucket-repo"
                    }
                }
            },
            "actor": {
        ...
        with patch('src.services.webhook_service.WebhookService') as mock_webhook_service:
            ...
            mock_instance.process_bitbucket_webhook = AsyncMock(return_value={
                "status": "processed",
                "commits_processed": 1,
    ...
    async def test_webhook_signature_validation(self, async_client: AsyncClient):
        ...
        assert valid_response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]
    ...
    @pytest.mark.asyncio
    async def test_webhook_repository_not_found_handling(self, async_client: AsyncClient):
    ...
    async def test_webhook_unsupported_event_handling(self, async_client: AsyncClient):
        ...
        signature = self._generate_github_signature(payload, webhook_config_payload["webhook_secret"])
    ...
    async def test_webhook_concurrent_processing(self, async_client: AsyncClient):
        ...
        successful_responses = [
            ...
            if hasattr(r, 'status_code') and r.status_code in [status.HTTP_200_OK, status.HTTP_202_ACCEPTED]
        ]
    ...
    async def test_webhook_rate_limiting(self, async_client: AsyncClient):
        ...
        assert any(code in [status.HTTP_200_OK, status.HTTP_202_ACCEPTED] for code in status_codes)
        
        # Some might be rate limited (429) depending on implementation
        rate_limited = [code for code in status_codes if code == status.HTTP_429_TOO_MANY_REQUESTS]
